...
//                     console.log(user)
//                     const authoritiesFromRoles = user.roleResponses.flatMap(role => role.authorityResponses);
//                     console.log(authoritiesFromRoles)
//                     const roles = authoritiesFromRoles.map((role) => role.libelle)
//                     console.log(roles)
//                     // Extraire les authorityResponses directement présents dans user
//                     const directAuthorities = user.authorityResponses.map((auth) => auth.authorityResponse.libelle);
//                     console.log(directAuthorities)
//                     // Combiner les deux tableaux
//                     const combinedAuthorities = [...directAuthorities, ...roles];
        ...
        keycloak.init({ onLoad: 'login-required' }).then(async (auth) => {
            if (auth) {
                ...
                try {
                    ...
                    // Extraire les authorityResponses directement présents dans user
                    const directAuthorities = user.authorityResponses.map((auth) => ({
                    ...
                    const authoritiesFromRoles = user.roleResponses.flatMap(role => role.authorityResponses.map((roleAuth) => ({
